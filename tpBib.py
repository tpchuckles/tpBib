...
consecutiveWordThreshold=40 # this is a good number. you want to avoid things like re-used (or common) acknowledgements, but still capture like-paragraphs
parallelWorkers=7	# how many cpus do you have? using them all might bog things down if you're trying to leave this running in the background
...
# Suppose a file moves. the filename is the key in the index, so we need to make a new entry in the index (with the new filename/location) with all the old contents for that index. AND, we need to look through all other index entries which might point to the old file! 
	...
	for k in index.keys():
		...
		for pointerKey in ["matches","checkedTextAgainst","checkedPixelsAgainst"]: 
			if oldkey in index[k][pointerKey]:
				...
				#print(k)
# Check all folders/subfolders/files/etc, record the file name timestamp and text into a log file?
index={}
if os.path.exists("tpBib.json"):					# load the old index
	print("loading previous index file")
	with open("tpBib.json") as f:
		index=json.load(f)
# if somehow we corrupt the json file, we can make one-time programmatic corrections here. 
#for f in index.keys():
...
# We don't actually need to do a longest-common-substring finding, because ALL we care about is chunks of words above the threshold in both papers. 
# for this, we can simply break one document into words, then compare chunks of several words and see if that chunk-of-words is in the other document.
# if we find a match, stop searching. if we don't find a match, keep sliding out chunk window across. 
	...
	for f1 in list(sorted(index.keys())):
		...
		for f2 in tqdm(list(index.keys())):
			...
			if f2 in index[f1]["checkedTextAgainst"]:
				continue
			#print("compare",f1,"to",f2) ; start=time.time()
			# if file 2 has fewer words, it's more efficient to scan through it instead! 
			text1=index[f1]["text"] ; words1=text1.split() ; len1=len(words1)
			text2=index[f2]["text"] ; words2=text2.split() ; len2=len(words2)
			if len2<len1:
			...
			"""
			match=False
			for i in range(len(words1)-threshold):
				for j in range(len(words2)-threshold):
					sa=words1[i:i+threshold]
					sb=words2[j:j+threshold]
					matches=[ sa[k]==sb[k] for k in range(threshold) ]
			...
			#print("took",time.time()-start,"s") #; sys.exit()
...
def manageDuplicates(ignoreAlreadySorted=True): # TODO how should we handle forked matches? A matched B and C, B only matched A, C only matched A (because B and C are pages out of A, for example). 
...
def reletter(): # if "authorname2013c.pdf" if the first rename to "authorname2013a.pdf" and so on
...
#			if "ouloukia" in f2:
#				unmatchEntries([f,f2])
#unlinkEntries(list(index.keys()))
#dupeSanityCheck()

def menu(options,save=True): # a list of doubles: text and function to be called
	while True:
